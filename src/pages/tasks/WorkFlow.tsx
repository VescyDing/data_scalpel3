...
import { Tooltip, Dropdown, Drawer, Space, Button, message, Flex, Modal } from 'antd'
import {
    SaveOutlined,
    AimOutlined,
    UndoOutlined,
...
export const createNode = (
    ...
    const node = {
        ...
        data: {
            ...nodeTmplate,
            category: categoryMap[nodeTmplate.type.split('.')[1]],
        },
    }
    ...
    useEffect(() => {
        ...
        const deleteNode = (id) => {
            ...
            Modal.confirm({
                ...
                okText: '确定',
                cancelText: '取消',
                onOk: () => graph.removeNode(id),
            })
        }

        const getTables = async (id: string) => { // 每个节点获取当前输入表和字段的方法
            const startNodes = graph.getNodes().filter((item) => item.data.type.split('.')[1] === 'input')
            const canvas = transDataOut(graph.toJSON());
            const inputSummaries = _.map(startNodes, node => ({
                nodeId: node.id,
                outputTables: node.data?.configuration?.items ?? []
            }))
            const res = await tasks.preRun({
                canvas,
                inputSummaries,
            })
            _.each(res.data, node => {
                if (node?.state == 'OK') {
                    graph.getCellById(node.nodeId).data.status = CellStatus.SUCCESS
                } else if (node?.state == 'ERROR') {
                    graph.getCellById(node.nodeId).data.status = CellStatus.ERROR
                }
            })
            return _.find(res.data, { nodeId: id })
        }
        ...
        const openDrawer = (...args) => {
            ...
            if (data) {
                ...
                if (data.type) {
                    _showDetail(false)
                    import(`./nodes/${_.last(data.type.split('.'))}.tsx`).then((module) => {
                        _showDetail(true)
                        _Detail(<module.default data={{ ...data, id: args[0].cell.id }} menu={getPlusDagMenu({ className: 'drawer-menu', clickPlusDragMenu, type: data.type })} callBack={(data) => onFormSubmit(data)} closeDrawer={() => {
                            graph.unselect(currentSelectNode.node)
                        }} deleteNode={deleteNode} getTables={async () => await getTables(args[0].cell.id)} />)
                    })
                }
            }
        }
